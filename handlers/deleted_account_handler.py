...
try:
    ...
    from pyrogram.errors import FloodWait, SessionPasswordNeeded, PhoneCodeExpired, PhoneCodeInvalid
...
async def cleanup_pyrogram(context: ContextTypes.DEFAULT_TYPE):
    """Stuck connections ko forcefully close karta hai"""
    app = context.user_data.get('pyrogram_app')
    if app:
        try:
            await app.disconnect()
        except: pass
    context.user_data.pop('pyrogram_app', None)
...
async def deleted_account_message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Priority -1 group handler to catch OTP/Phone and block Spam Model"""
    ...
    if session_key not in sessions:
        try:
            await context.bot.send_message(
                chat_id=user.id,
                text=f"📌 **Group: {chat.title}**\nSetup ke liye apna phone number bhejein (+countrycode):"
            )
...
async def handle_phone_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await cleanup_pyrogram(context) # Purane connections clear karein
...
async def handle_otp_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Code Expire ho gaya! Dobara number bhejein:")
        context.user_data['awaiting_phone'] = True
        await cleanup_pyrogram(context)
    except Exception as e:
...
def register_deleted_account_handlers(app):
    ...
    # Priority group -1 fixes the Spam Model conflict
